...
def keys_kernel2d(x: np.array, offset: float, width: float, alpha: float = -0.5) -> np.array:
    ...
    y = np.abs(y)
    return (
        ((alpha + 2) * x**3 - (alpha + 3) * x**2 + 1) * ((alpha + 2) * y**3 - (alpha + 3) * y**2 + 1)
        * (x >= 0) * (x < 1) * (y >= 0) * (y < 1)
        + (
            alpha * x**3 - 5 * alpha * x**2 + 8 * alpha * x - 4 * alpha
        ) * ((alpha + 2) * y**3 - (alpha + 3) * y**2 + 1) * (x >= 1) * (x < 2) * (y >= 0) * (y < 1)
        + (
            alpha * y**3 - 5 * alpha * y**2 + 8 * alpha * y - 4 * alpha
        ) * ((alpha + 2) * x**3 - (alpha + 3) * x**2 + 1) * (y >= 1) * (y < 2) * (x >= 0) * (x < 1)
        + (
            alpha * x**3 - 5 * alpha * x**2 + 8 * alpha * x - 4 * alpha
        ) * (alpha * y**3 - 5 * alpha * y**2 + 8 * alpha * y - 4 * alpha) * (x >= 1) * (x < 2) * (y >= 1) * (y < 2)
    )
